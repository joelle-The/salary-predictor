...
X_columns_after_ohe=['age', 'capital-gain', 'capital-loss', 'hours-per-week',
       ...
       'native-country_ Yugoslavia', 'education_ 11th', 'education_ 12th',
...
with st.sidebar:
    ...
    test_user_input=pd.DataFrame(data,index=[0])
    test_user_input = pd.get_dummies(test_user_input, columns=['workclass', 'marital-status', 'occupation', 'relationship', 'race', 'sex', 'native-country', 'education'])
    test_user_input = test_user_input.reindex(columns=X_columns_after_ohe, fill_value=0)
...
test_user_input.iloc[0,[0,3]]=scaler.transform(test_user_input.iloc[0,[0,3]].values.reshape(1, -1))


prediction = model.predict(test_user_input)
prediction_proba = model.predict_proba(test_user_input)

#show the prediction result
result = prediction[0]
confidence = np.max(prediction_proba[0]) * 100
# st.markdown("### Prediction Result")
# st.markdown(f"**Predicted Income Level:** {result}")
...
with col2:
    ...
    if result == '> $50K':
        st.markdown(f"""
        <div style="background: #F0E4D3; padding: 2rem; border-radius: 15px; text-align: center; color: #2d3436;">
